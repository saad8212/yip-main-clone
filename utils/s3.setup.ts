import { S3 } from "aws-sdk";

const bucketName = process.env.NEXT_PUBLIC_AWS_BUCKET_NAME;
const accessKeyId = process.env.NEXT_PUBLIC_AWS_ACCESS_KEY;
const secretAccessKey = process.env.NEXT_PUBLIC_AWS_SECRET_ACCESS_KEY;
const region = process.env.NEXT_PUBLIC_AWS_BUCKET_REGION;

type Params = {
    Bucket?: string;
    Key: string;
    Body?: any;
    ContentType?: "image/webp";
};

const s3 = new S3({
    accessKeyId,
    secretAccessKey,
    region: region
});

const uploadFile = (img: any, imgName: string, cb: CallableFunction) => {

    const params: Params = {
        Bucket: bucketName,
        Key: imgName,
        Body: img,
        ContentType: 'image/webp'
    };
    return new Promise((resolve, reject) => {
        s3.upload(params as any, function (err: Error, data: any) {
            if (err) {
                reject(err);
            } else {
                resolve(data);
            }
        })
            .on("httpUploadProgress", (p) => {
                const progress = Math.round((p.loaded / p.total) * 100);
                cb(progress);
            });
    });
};

const getImage = (key: string) => {
    const params: Params = {
        Bucket: bucketName,
        Key: key
    };

    return new Promise((resolve, reject) => {
        s3.getObject(params as any, function (err, data) {
            if (err) {
                reject(err)
            } else {
                resolve(data.Body);
            }
        });
    });

};

const deleteImage = (key: string) => {
    const params = {
        Bucket: bucketName,
        Key: key
    };
    return new Promise((resolve, reject) => {
        s3.deleteObject(params as any, function (err, data) {
            if (err) {
                reject(err);
            } else {
                resolve('Image deleted successfully.');
            }
        });
    });
};

export { uploadFile, deleteImage, getImage };


